...
from ml.entity.artifact import DataTransformationArtifact,ModelTrainerArtifact
from ml.entity.model_train_config import ModelTrainerConfig

import mlflow

from ml.utils.ml_utils.model.estimator import NetworkModel
from ml.utils.main_utils.utils import save_object,load_object
from ml.utils.main_utils.utils import load_numpy_array_data, evaluate_models
...
class ModelTrainer:
    ...
    def train_model(self,Xtrain,ytrain,Xtest,ytest)->ModelTrainerArtifact:
        try:
            ...
            # --- Compute per-model accuracy and save metrics ---

            all_model_accuracies = {}
            for model_name, model in models.items():
                model.set_params(**report_param[model_name])
                model.fit(Xtrain, ytrain)
                y_pred_test = model.predict(Xtest)
                acc = round(accuracy_score(ytest, y_pred_test) * 100, 1)
                all_model_accuracies[model_name] = acc

            best_acc = all_model_accuracies[best_model_name]

            metrics_payload = {
                "model_accuracies": all_model_accuracies,
                "best_model_name": best_model_name,
                "best_model_accuracy": best_acc,
                "f1_score": round(classification_test_metric.f1_score, 4),
                "precision": round(classification_test_metric.precision_score, 4),
                "recall": round(classification_test_metric.recall_score, 4),
            }

            os.makedirs("models", exist_ok=True)
            with open("models/metrics.json", "w") as f:
                json.dump(metrics_payload, f)
            logging.info(f"Metrics saved to models/metrics.json: {metrics_payload}")
            # --- End save metrics ---
