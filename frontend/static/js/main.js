// Tab Navigation
        ...
        let accuracyChartInstance = null;  // keep reference to update later
            ...
            fetch('/api/v1/train')
                .then(response => {
                    ...
                    if (response.ok) {
                        ...
                        const modelAccuraciesCard = document.getElementById('modelAccuraciesCard');
                        if (modelAccuraciesCard) {
                            modelAccuraciesCard.style.filter = 'none';
                            modelAccuraciesCard.style.opacity = '1';
                            modelAccuraciesCard.style.pointerEvents = 'auto';
                            modelAccuraciesCard.style.userSelect = 'auto';
                        }
                        const accuraciesOverlay = document.getElementById('modelAccuraciesOverlay');
                        if (accuraciesOverlay) accuraciesOverlay.style.display = 'none';

                        // Fetch real metrics and update UI
        ...
        // Fetch real metrics from backend and update chart + cards
        function fetchAndRenderMetrics() {
            fetch('/api/v1/metrics')
                ...
                .then(resp => {
                    ...
                    const d = resp.data;
                    const accuracies = d.model_accuracies; // { "Random Forest": 97.1, ... }

                    // Sort models descending for chart display
                    const sortedModels = Object.entries(accuracies).sort((a, b) => b[1] - a[1]);
                    const labels = sortedModels.map(m => m[0]);
                    const data   = sortedModels.map(m => m[1]);

                    // Color palette - first bar brighter, rest progressive
                    const chartColors = [
                        '#0071e3', '#4facfe', '#88c5f7', '#a8d5f7', '#c0e0f7', '#e1effa'
                    ];

                    // Update Chart.js accuracy chart
                    if (accuracyChartInstance) {
                        accuracyChartInstance.data.labels = labels;
                        accuracyChartInstance.data.datasets[0].data = data;
                        accuracyChartInstance.data.datasets[0].backgroundColor = chartColors.slice(0, labels.length);
                        const minAcc = Math.max(50, Math.min(...data) - 5);
                        accuracyChartInstance.options.scales.y.min = minAcc;
                        accuracyChartInstance.update();
                    }

                    // Update Best Model banner
                    const bestName = d.best_model_name;
                    const bestAcc  = d.best_model_accuracy;
                    const bestNameEl = document.getElementById('bestModelName');
                    const bestScoreEl = document.getElementById('bestModelScore');
                    const bestDescEl  = document.getElementById('bestModelDesc');
                    if (bestNameEl)  bestNameEl.innerText  = `🏆 Best Model: ${bestName}`;
                    if (bestScoreEl) bestScoreEl.innerText = `${bestAcc}%`;
                    if (bestDescEl)  bestDescEl.innerText  = `Achieved the highest test accuracy of ${bestAcc}% with F1 Score: ${d.f1_score}, Precision: ${d.precision}, Recall: ${d.recall}.`;

                    // Rebuild model cards grid
                    const grid = document.getElementById('modelsGrid');
                    if (grid) {
                        // Show all models except the best one (already shown in banner)
                        const others = sortedModels.filter(m => m[0] !== bestName);
                        grid.innerHTML = others.map(([name, acc]) => `
                            <div class="model-card">
                                <h4>${name}</h4>
                                <div class="model-accuracy">${acc}%</div>
                            </div>
                        `).join('');
                    }
                })
